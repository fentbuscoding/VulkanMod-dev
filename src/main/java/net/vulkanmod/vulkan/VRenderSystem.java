...
public abstract class VRenderSystem {
    ...
    public static void updateScreenSize() {
        ...
        if (width != cachedWidth || height != cachedHeight) {
            screenSize.putFloat(0, (float) width);
            screenSize.putFloat(4, (float) height);
            cachedWidth = width;
            cachedHeight = height;
        }
    }
    ...
    public static void setPrimitiveTopologyGL(final int mode) {
        VRenderSystem.topology = switch (mode) {
            case GL15.GL_LINES, GL15.GL_LINE_STRIP  -> VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
            case GL15.GL_TRIANGLE_FAN, GL15.GL_TRIANGLES, GL15.GL_TRIANGLE_STRIP -> VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
    ...
    public static void setPolygonModeGL(final int mode) {
        VRenderSystem.polygonMode = switch (mode) {
            case GL15.GL_POINT -> VK_POLYGON_MODE_POINT;
            case GL15.GL_LINE -> VK_POLYGON_MODE_LINE;
            case GL15.GL_FILL -> VK_POLYGON_MODE_FILL;
